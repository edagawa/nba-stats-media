...
def generate_player_pages(env, scoring_timeline_data, df_s1_raw, df_s2_raw):
    ...
    def generate_player_comment(player_name, season_str_long, player_season_timeline):
        ...
        if best_quarter == 4 and (q4_points / total_points) > 0.3:
            ...
            comment_parts.append(f"総得点の{total_points:.0f}点のうち、実に{q4_points:.0f}点をこの時間帯で記録しており、勝負強さが際立っています。")
        elif best_quarter == 1 and (q1_points / total_points) > 0.3:
            comment_parts.append(f"{player_name}選手は、試合序盤から積極的に得点を狙うスタートダッシュ型の選手です。")
            comment_parts.append(f"第1クォーターに最も多くの得点を挙げる傾向があり、チームに勢いをもたらします。")
        else:
            comment_parts.append(f"{player_name}選手は、シーズンを通して安定したパフォーマンスを見せ、特に第{best_quarter}クォーターで最も多くの得点を記録しています。")
        comment_parts.append(f"強みは{primary_style}です。")
    ...
    for index, player_data in df_merged.iterrows():
        ...
        try:
            ...
            for season_str_short in ["23-24", "24-25"]:
                ...
                attempts_pivot = player_timeline.pivot_table(index='absolute_minute', columns=['SHOT_TYPE', 'MADE_FLAG'], aggfunc='size', fill_value=0)
                ...
                ax.bar(attempts_agg['absolute_minute'], miss_3pt, bottom=bottom, color='#aec7e8', label='3PT Miss'); bottom += miss_3pt
                ax.bar(attempts_agg['absolute_minute'], miss_2pt, bottom=bottom, color='#ffbb78', label='2PT Miss'); bottom += miss_2pt
                ax.bar(attempts_agg['absolute_minute'], miss_ft, bottom=bottom, color='#ff9896', label='FT Miss'); bottom += miss_ft
                made_3pt = attempts_agg.get('3PT_1', 0); made_2pt = attempts_agg.get('2PT_1', 0); made_ft = attempts_agg.get('FT_1', 0)
                ax.bar(attempts_agg['absolute_minute'], made_3pt, bottom=bottom, color='#1f77b4', label='3PT Made'); bottom += made_3pt
                ax.bar(attempts_agg['absolute_minute'], made_2pt, bottom=bottom, color='#ff7f0e', label='2PT Made'); bottom += made_2pt
                ...
                ax.bar(points_agg['absolute_minute'], points_agg.get('3PT', 0), color='#1f77b4', label='3-Pointers'); ax.bar(points_agg['absolute_minute'], points_agg.get('2PT', 0), bottom=points_agg.get('3PT', 0), color='#ff7f0e', label='2-Pointers'); ax.bar(points_agg['absolute_minute'], points_agg.get('FT', 0), bottom=points_agg.get('3PT', 0) + points_agg.get('2PT', 0), color='#2ca02c', label='Free Throws')
