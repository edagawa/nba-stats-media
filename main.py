...
def generate_season_player_index(env, base_path, season_str, df_current, df_previous):
    ...
    for stat, names in player_stats_to_show.items():
        ...
        player_data_list = [{'Player': row['Player'], 'url': f"{base_path}/players/{re.sub(r'[\\/*?:\"<>|]', '', row['Player']).replace(' ', '_')}.html", 'value': row[stat_key_current], 'change': row[stat_key_change]} for _, row in top_10_players.iterrows()]
