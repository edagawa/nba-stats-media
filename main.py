...
# (CONFERENCE_STRUCTURE と generate_season_pages は変更なし)
...
def generate_season_pages(season_string, team_stats_df, stat_pages_info_main):
    ...
    for index, row in team_stats_df.iterrows():
        team_name = row['Team']
        # ★★★ サマリーテキストにNET EFFを追加 ★★★
        summary_text = "{0}は、{1}シーズンを平均{2:.1f}得点、NET RTG {3:.2f}で終えました。".format(team_name, season_string, row['PTS'], row['NET EFF'])
...
def generate_comparison_pages(df_s1, df_s2):
    ...
    # ★★★ 比較する指標に新しい指標を追加 ★★★
    stats_to_compare = ['PTS', 'REB', 'AST', 'OFF EFF', 'DEF EFF', 'NET EFF']
    
    stats_to_generate = { 'PTS': 'Points Per Game', 'REB': 'Rebounds Per Game', 'AST': 'Assists Per Game', 'STL': 'Steals Per Game', 'BLK': 'Blocks Per Game', 'FG%': 'Field Goal Pct', 'OFF EFF': 'Offensive Efficiency', 'DEF EFF': 'Defensive Efficiency', 'NET EFF': 'Net Rating' }
