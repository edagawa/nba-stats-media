...
def generate_season_pages(season_string, team_stats_df, stat_pages_info):
    """指定されたシーズンの全ページを生成する"""
    ...
    for index, row in team_stats_df.iterrows():
        ...
        report_data = {
            'team_name': team_name, 'season_string': season_string, 'summary_text': summary_text,
            'team_stats_html': row.to_frame().T.to_html(index=False), 'main_page_url': '../../index.html'
        }
    ...
    # ★★★ ここから修正箇所 ★★★
    # シーズンごとのページ用に、階層構造のリンクリストを再構築
    season_teams_structured = {}
    for conf, divisions in CONFERENCE_STRUCTURE.items():
        season_teams_structured[conf] = {}
        for div, teams in divisions.items():
            season_teams_structured[conf][div] = []
            for team in teams:
                # リンク先をシーズンごとのページに変更
                url = "./teams/{0}_{1}_season.html".format(team.replace(' ', '_'), season_string)
                season_teams_structured[conf][div].append({'name': team, 'url': url})
    
    # 正しい変数名でテンプレートに渡す
    html_content = index_template.render(
        all_teams_structured=season_teams_structured, 
...
def generate_comparison_pages(df_s1, df_s2, all_teams_structured, stat_pages_info):
    # (この関数は変更なし)
    print("--- 比較ページの生成開始 ---")
    df_s1 = df_s1.set_index('Team'); df_s2 = df_s2.set_index('Team')
    all_teams = list(df_s1.index.union(df_s2.index))
    template_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates')
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(template_dir))
...
def generate_main_index(all_teams_structured, stat_pages_info):
    # (この関数は変更なし)
    print("--- メインインデックスページの生成開始 ---")
    template_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates')
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(template_dir))
    index_template = env.get_template('index_template.html')
    html_content = index_template.render(season_string="チーム比較", all_teams_structured=all_teams_structured, stat_pages=stat_pages_info)
    with open(os.path.join("output", "index.html"), "w", encoding="utf-8") as f: f.write(html_content)
    print("--- メインインデックスページの生成完了 ---")

def generate_stat_comparison_pages(df_s1, df_s2, all_teams_structured, stat_pages_info):
    # (この関数は変更なし)
    print("--- 指標別比較ページの生成開始 ---")
    ...
    stats_to_generate = { 'PTS': 'Points Per Game', 'REB': 'Rebounds Per Game', 'AST': 'Assists Per Game', 'STL': 'Steals Per Game', 'BLK': 'Blocks Per Game', 'FG%': 'Field Goal Pct' }
    for stat_en, stat_full_en in stats_to_generate.items():
        safe_stat_en = stat_en.replace('%', '_PCT')
        ...
        rects1 = ax.bar(x - width/2, stats1, width, label='2023-24'); rects2 = ax.bar(x + width/2, stats2, width, label='2024-25')
        ...
        render_data = {
            'stat_name_jp': stat_full_en, 'stat_name_en': safe_stat_en,
            'graph_url': '../images/stat_{0}.svg'.format(safe_stat_en),
            'data_table_html': sorted_df[['Team', f'{stat_en}_s1', f'{stat_en}_s2']].to_html(index=False),
            'all_teams_structured': all_teams_structured, 'stat_pages': stat_pages_info
        }
...
def main():
    ...
    stats_to_generate = { 'PTS': 'Points Per Game', 'REB': 'Rebounds Per Game', 'AST': 'Assists Per Game', 'STL': 'Steals Per Game', 'BLK': 'Blocks Per Game', 'FG%': 'Field Goal Pct' }
    stat_pages_info = [{'name': en_full, 'url': './stats/{0}.html'.format(en_short.replace('%', '_PCT'))} for en_short, en_full in stats_to_generate.items()]
    all_teams_structured = {}
    for conf, divisions in CONFERENCE_STRUCTURE.items():
        all_teams_structured[conf] = {}
        for div, teams in divisions.items():
            all_teams_structured[conf][div] = [{'name': team, 'url': './teams/{0}.html'.format(team.replace(' ', '_'))} for team in teams]

    # ★★★ generate_season_pagesに関数を渡すように修正 ★★★
    generate_season_pages("2023-24", df_s1, stat_pages_info)
    generate_season_pages("2024-25", df_s2, stat_pages_info)
    
    generate_comparison_pages(df_s1, df_s2, all_teams_structured, stat_pages_info)
    generate_stat_comparison_pages(df_s1, df_s2, all_teams_structured, stat_pages_info)
    generate_main_index(all_teams_structured, stat_pages_info)
