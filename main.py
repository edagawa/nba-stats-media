...
def generate_season_pages(season_string, team_stats_df, stat_pages_info_main):
    ...
    # このページはトップページではないので、チーム比較へのリンクは不要。指標別リンクのみでOK
    ...
    html_content = index_template.render(
        ...
        stat_pages=stat_pages_info_seasonal
    )
...
def generate_comparison_pages(df_s1, df_s2):
    ...
    stat_pages_info_footer = [{'name': en_full, 'url': '../stats/{0}.html'.format(en_short.replace('%', '_PCT'))} for en_short, en_full in stats_to_generate.items()]
    ...
    for team in all_teams:
        try:
            ...
            render_data = { 
                ...
                'stats_s1': df_s1.loc[team].to_frame().to_html(), 'stats_s2': df_s2.loc[team].to_frame().to_html(),
                'all_teams_structured': all_teams_structured_footer, 'stat_pages': stat_pages_info_footer
            }
...
def generate_stat_comparison_pages(df_s1, df_s2):
    ...
    stat_pages_info_footer = [{'name': en_full, 'url': './{0}.html'.format(en_short.replace('%', '_PCT'))} for en_short, en_full in stats_to_generate.items()]
...
def main():
    ...
    stats_to_generate = { 'PTS': 'Points Per Game', 'REB': 'Rebounds Per Game', 'AST': 'Assists Per Game', 'STL': 'Steals Per Game', 'BLK': 'Blocks Per Game', 'FG%': 'Field Goal Pct' }
    stat_pages_info_main = [{'name': en_full, 'url': './stats/{0}.html'.format(en_short.replace('%', '_PCT'))} for en_short, en_full in stats_to_generate.items()]
